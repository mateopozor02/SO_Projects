...
public class Supplier implements Runnable {
    ...
    public void run() {
        for (int i = 0; i < iterations; i++) {
            try {
                Thread.sleep(P3.randInt(100));
            } catch (Exception e) {
                e.printStackTrace();
            }
            Grain g = Grain.randChoice();
            int amount = P3.randInt(1,10);
            delivered.change(g, amount);
            P3.debug("delivering %d %s of %s to the %s broker",
                amount, (amount == 1 ? "bushel" : "bushels"), g, g);
            try {
                P3.specialist(g).deliver(amount);
            } catch (InterruptedException ex) {
                Logger.getLogger(Supplier.class.getName()).log(Level.SEVERE, null, ex);
            }

        }
        P3.debug("Supplier shutting down ...");
    } // run()
} // Supplier
